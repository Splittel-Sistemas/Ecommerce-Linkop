...
var validarEmailRepetido1 = false;

$("#regEmail1").change(function(){
	...
	$.ajax({
		...
		success:function(respuesta){
			...
			if(respuesta == "false"){
				...
				$(".alert").remove();
				validarEmailRepetido1 = false;

			}else{

				var modo = JSON.parse(respuesta).modo;
				
				if(modo == "directo" || modo == "invitado" ){

					modo = "esta página";
				}

				$("#regEmail1").parent().before('<div class="alert alert-warning"><strong>ERROR:</strong> El correo electrónico ya existe en la base de datos, fue registrado a través de '+modo+', por favor ingrese otro diferente</div>')

					validarEmailRepetido1 = true;
...
VALIDAR EL REGISTRO DE USUARIO
=============================================*/
function registroUsuarioInvitado(){

	/*=============================================
	VALIDAR EL NOMBRE
	=============================================*/

	var nombre = $("#regUsuario1").val();
	...
	if(nombre != ""){
		...
		if(!expresion.test(nombre)){
			...
			$("#regUsuario1").parent().before('<div class="alert alert-warning"><strong>ERROR:</strong> No se permiten números ni caracteres especiales</div>')
		...
		$("#regUsuario1").parent().before('<div class="alert alert-warning"><strong>ATENCIÓN:</strong> Este campo es obligatorio</div>')
	...
	if(email != ""){
		...
		if(!expresion.test(email)){
			...
			$("#regEmail1").parent().before('<div class="alert alert-warning"><strong>ERROR:</strong> Escriba correctamente el correo electrónico</div>')

			return false;

		}

		if(validarEmailRepetido1){

			$("#regEmail1").parent().before('<div class="alert alert-danger"><strong>ERROR:</strong> El correo electrónico ya existe en la base de datos, por favor ingrese otro diferente</div>')
		...
		$("#regEmail1").parent().before('<div class="alert alert-warning"><strong>ATENCIÓN:</strong> Este campo es obligatorio</div>')
